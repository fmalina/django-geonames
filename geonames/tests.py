...
from geonames.models import (
    Timezone, Language, Currency, Country,
...
def create_test_town():
    ...
    aa = Admin2Code.objects.create(
        geonameid=1, code='greater-london', name='Greater London', country=gb, admin1=en, slug='ldn')
    locality = Locality.objects.create(
        status=1, geonameid=1, population=100, name='London', long_name='London',
        country=gb, admin2=aa, lat=51.508, lon=-0.125, modification_date=datetime.today())
    return locality
...
class SimpleTest(TestCase):
    def setUp(self):
        ...
        # Alternate names
        self.altName1 = AlternateName.objects.create(alternatenameid=1, locality=self.locality1, name="Loc1 alt")

    def test_country_deleted(self):
        country3 = Country.objects.create(code='C3', name="Country3", currency=self.currency1, status=0)
        all_countries = Country.objects.public()
        self.assertEqual(all_countries.count(), 2)
        self.assertNotIn(country3, all_countries)
        all_w_deleted = Country.objects.all()
    ...
    def test_adm2_country_consistency(self):
        ...
        self.assertRaises(ValueError, adm2.save)
    ...
    def test_save_locations(self):
        ...
        self.assertRaisesMessage(
            ValueError,
            "Duplicated locality long name 'Loc1, a2c1 name, a1c1 name'",
            Locality.objects.create,
            geonameid=1300, name="Loc1", country=self.country1,
            admin1=self.admin1code1, admin2=self.admin2code1, timezone=self.tz1,
            population=1, lat=1, lon=1, modification_date="2012-1-1"
        )
        # Check country and adm zones consistency
        self.assertRaisesMessage(
            ValueError,
            """The country '(C2) Country2'
                from the Admin1 'Country2 > a1c3 name' is different
                than the country '(C1) Country1'
                from the locality 'Loc1, a2c1 name, a1c3 name'""",
            Locality.objects.create,
            geonameid=1300, name="Loc1", country=self.country1,
            admin1=self.admin1code3, admin2=self.admin2code1, timezone=self.tz1,
            population=1, lat=1, lon=1, modification_date="2012-1-1"
        )
        self.assertRaisesMessage(
            ValueError,
            """The country '(C2) Country2'
                from the Admin2 'Country2 > a1c3 name > a2c5 name'
                is different than the country '(C1) Country1'
                from the locality 'Loc1, a2c5 name, a1c1 name'""",
            Locality.objects.create,
            geonameid=1300, name="Loc1", country=self.country1,
            admin1=self.admin1code1, admin2=self.admin2code5, timezone=self.tz1,
            population=1, lat=1, lon=1, modification_date="2012-1-1"
        )

    def test_localities_deleted(self):
        locality = Locality.objects.create(
            geonameid=1300, name="Loc1_deleted", country=self.country1,
            admin1=self.admin1code1, admin2=self.admin2code1, timezone=self.tz1,
            population=1, lat=1, lon=1, modification_date="2012-1-1", status=0
        )
        all_ls = Locality.objects.public()
        self.assertEqual(all_ls.count(), 3)
        self.assertNotIn(locality, all_ls)
        all_with_deleted = Locality.objects.all()
        self.assertEqual(all_with_deleted.count(), 4)
    ...
    def test_locality_search(self):
        ...
        locality = Locality.objects.create(
            geonameid=1300, name="Loc1", country=self.country2,
            admin1=self.admin1code3, admin2=self.admin2code5, timezone=self.tz1,
            population=1, lat=1, lon=1, modification_date="2012-1-1"
        )
        result = self.country1.search_locality("Loc1")
        self.assertIn(self.locality1, result)
    ...
    def test_gis(self):
        ...
        if not getattr(settings, 'GIS_LIBRARIES', False):
            return
